...
class LogisticRegression(Problem):
    ...
    def _init(self, result_queue=None):
        ...
        norm = np.linalg.norm(self.X_train, 2) / (2 * np.sqrt(self.m_total)) # Upper bound of the hessian
        self.X_train /= norm
        self.X /= norm

        if self.kappa is not None:
            log.info('Computing min')
            x_min, count = NAG(self.grad, np.random.randn(self.dim), self.L, self.sigma, n_iters=5000, eps=1e-10)
            log.info(f'NAG ran for {count} iterations')
            f_min = self.f(x_min)
            log.info(f'f_min = {f_min}')
            log.info(f'grad_f(x_min) = {np.linalg.norm(self.grad(x_min))}')
    ...
    def h(self, w, i=None, j=None, split='train'):
        ...
        if i is None:  # Return the function value
            tmp = X.dot(w)
            return -np.sum(
                (Y - 1) * tmp - np.log1p(np.exp(-tmp))
            ) / X.shape[0] + np.sum(w**2) * self.LAMBDA / 2

        elif j is None:  # Return the function value in machine i
            tmp = self.X[i].dot(w)
            return -np.sum((self.Y[i] - 1) * tmp - np.log1p(np.exp(-tmp))) / self.m + np.sum(w**2) * self.LAMBDA / 2
        else:  # Return the gradient of sample j in machine i
            tmp = self.X[i][j].dot(w)
            return -((self.Y[i][j] - 1) * tmp - np.log1p(np.exp(-tmp))) + np.sum(w**2) * self.LAMBDA / 2
